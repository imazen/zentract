...
//! (`docs/public-api/<crate>.txt`, one per workspace lib crate) on every
...
/// All workspace crates: the `zentract-types` / `zentract-abi` surfaces are
/// the dlopen plugin compatibility contract, so every lib crate is
/// snapshotted.
const CRATES: &[&str] = &["zentract-types", "zentract-abi", "zentract-api"];
    ...
    for package in CRATES {
        ...
        doc.push_str(&format!(
            "# {package} public API surface\n\
             # Generated by zentract-api/tests/public_api_doc.rs via `cargo public-api --simplified`\n\
